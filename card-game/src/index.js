  ...
  handleClick() {
    ...
    if (userCards[0].value > compCards[0].value) {
      ...
      document.getElementById(
        "user-card-about"
      ).innerHTML = `User cards remaining: ${userCards.length} `;
      document.getElementById(
        "comp-card-about"
      ).innerHTML = `Computer cards remaining: ${compCards.length}`;
      ...
      if (userCards[3].value >= compCards[3].value) {
        document.getElementById(
          "user-card-about"
        ).innerHTML = `User cards remaining: ${userCards.length + 4} `;
        document.getElementById(
          "comp-card-about"
        ).innerHTML = `Computer cards remaining: ${compCards.length - 4}`;
      } else {
        document.getElementById(
          "comp-card-about"
        ).innerHTML = `Computer cards remaining: ${compCards.length + 4}`;
